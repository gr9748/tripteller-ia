          ...
          {plan.ai_response ? (
            <div className="space-y-6">
              ...
              {plan.ai_response.summary && (
                <div className="bg-blue-50/50 dark:bg-blue-900/20 p-6 rounded-lg">
                  <h3 className="font-medium mb-2 text-lg text-blue-700 dark:text-blue-300">Trip Summary</h3>
                  <p className="text-slate-700 dark:text-slate-300">{plan.ai_response.summary}</p>
                </div>
              )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                {plan.ai_response.flights && plan.ai_response.flights.length > 0 && (
                  <div className="bg-sky-50/50 dark:bg-sky-900/20 p-6 rounded-lg">
                    <h3 className="font-medium mb-3 text-lg text-sky-700 dark:text-sky-300">Recommended Flights</h3>
                    <div className="space-y-3">
                      {plan.ai_response.flights.slice(0, 3).map((flight: any, index: number) => (
                        <div key={index} className="p-3 border border-sky-100 dark:border-sky-800 rounded-md">
                ...
                {plan.ai_response.accommodations && plan.ai_response.accommodations.length > 0 && (
                  <div className="bg-amber-50/50 dark:bg-amber-900/20 p-6 rounded-lg">
                    <h3 className="font-medium mb-3 text-lg text-amber-700 dark:text-amber-300">Recommended Stays</h3>
                    <div className="space-y-3">
                      {plan.ai_response.accommodations.slice(0, 3).map((accommodation: any, index: number) => (
                        <div key={index} className="p-3 border border-amber-100 dark:border-amber-800 rounded-md">
              ...
              {/* Action buttons */}
              <div className="flex flex-wrap gap-4 pt-6 mt-4 border-t border-gray-200 dark:border-gray-700">
                <Button 
                  variant="default" 
                  size="lg"
